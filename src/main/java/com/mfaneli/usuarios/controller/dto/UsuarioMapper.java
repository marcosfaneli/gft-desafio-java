...
public interface UsuarioMapper {
    ...
    @Mapping(target = "habilidades", expression = "java(mapList(request.getHabilidades()))")
    @Mapping(target = "id", source = "id")
    @Mapping(target = "dataNascimento", source = "request.dataNascimento", dateFormat = "dd/MM/yyyy")
    @Mapping(target = "email", source = "request.email", defaultExpression = "java(request.getEmail().toLowerCase())")
    @Mapping(target = "nome", source = "request.nome", defaultExpression = "java(request.getNome().toUpperCase())")
    @Mapping(target = "dataCriacao", ignore = true)
    @Mapping(target = "dataAtualizacao", ignore = true)
    Usuario map(UsuarioRequest request, Integer id);
